...
#takes a pose and returns a list containing lists of consecutive residues with helical torsion angles
def helixResList (pose):

    #create and initialize residue selector for alpha helical torsion angles
    bsc = pyrosetta.rosetta.core.select.residue_selector.BinSelectorCreator()
    ...
    #use the residue selector to build a list of residue ranges where more than one consecutive residue has helical torsions.
    heliNums = zip(range(heliVec.capacity()), heliVec)
    heliter = itertools.groupby(heliNums,lambda x: x[1])
    heliOut = []
    for k,v in heliter:
        a = []
        if k:
...
#returns the pos numbered first and last residue number of the longest helix of the pose in a two member list
