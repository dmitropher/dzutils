...
def rt_from_chis(*chis, base_atoms=(), target_atoms=(), rotamer_rt_array=None):
...
def dask_rt_from_chis(chis, target_atoms):
    return rt_from_chis(*chis, target_atoms=target_atoms)
...
def main(
    ...
    residue = pyrosetta.rosetta.core.conformation.Residue(res_type, True)
    possible_rt_bases = [list(base) for base in pres_bases(residue)]
    ...
    rts, chis_index, alignment_atoms = zip(
        *[
            (
                rt_from_chis(
                    *chi_set, target_atoms=atoms, rotamer_rt_array=rotamer_rt
                ),
                chi_set,
                atoms,
            )
    ...
    client = Client(scheduler_file="/home/dzorine/jupyter_wd/scheduler.json")
    ...
    for i in range(1, cycle_depth + 1):
        ...
        for chis in chi_sets:
            for atoms in possible_rt_bases:
                ...
                # )
        atom_bag = db.from_sequence(batch_new_atoms, partition_size=10)
        chi_bag = db.from_sequence(batch_new_chis, partition_size=10)
        rt_bag = db.map(
            dask_rt_from_chis,
            chi_bag,
            target_atoms=atom_bag,
            rotamer_rt_array=rotamer_rt,
        )
        rt_list = rt_bag.compute()

        batch_new_keys = binner.get_bin_index(np.array(rt_list))
